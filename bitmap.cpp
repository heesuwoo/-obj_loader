...
namespace
{
    ...
    // TGA 파일 헤더 구조. *must* 바이트는 정렬이어야 한다.
    struct TgaHeader
    {
        BYTE idLength;
        BYTE colormapType;
    ...
    // TGA 파일 바닥글 구조.  *must* 바이트는 정렬이어야 한다.
    struct TgaFooter
    {
...
{
	//이미지 출력할 위치의 핸들,출력할 x좌표, y좌표, 너비, 높이, 이미지핸들(받은 이미지),가져올 이미지 시작점x좌표, y좌표, 잘라낼 너비, 높이, 출력방법 
    StretchBlt(hdcDest, 0, 0, width, height, dc, 0, 0, width, height, SRCCOPY); //원본 이미지 출력
}
